...
string prefix;
const string suffix("_variational.dat");
...
double external_potential(double x)
{
	return x*x * (x*x - 5./2);
}

double external_potential_prime(double x)
{
	return x * (4.*x*x - 5.);
}

double external_potential_second(double x)
{
	return 12.*x*x - 5.;
...
{
	double mu(0.82), sigma(0.62);
	double plus  = exp(-0.5 * pow((x - mu) / sigma, 2));
	double minus = exp(-0.5 * pow((x + mu) / sigma, 2));
	return -0.5 * pow(sigma, -2) * (
			(plus + minus) +
			pow(sigma, -2) * (pow(x - mu, 2) * minus + pow(x + mu, 2) * plus)
			);
}
...
{
	ofstream out(prefix + "potential" + suffix);
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out(prefix + "kinetic" + suffix);
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out(prefix + "probability" + suffix);
