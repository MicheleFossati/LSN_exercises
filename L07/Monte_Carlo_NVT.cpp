...
//Read initial configuration
  ...
  for(int i=0; i<nbins; i++){
    g_norm[i] = rho*npart*4*pi/3*pow(bin_size,3) * (pow(i+1,3) - pow(i,3));
  }
  
  ofstream Gofr; // here we print the values of r for g(r)
  Gofr.open("block_gofr.dat",ios::app);
  for(int i=0; i<nbins; i++){
    Gofr << setw(print_wd) << bin_size*(i+0.5);
  }
  Gofr << endl;
  Gofr.close();
...
//update of the histogram of g(r), maybe you should restrict r to r_cut
     if (dr<box/2)
       walker[igofr + static_cast<int>(dr/bin_size)] +=2;
...
// contribution to energy and virial
  ...
  for(int i=igofr; i< n_props; i++) walker[i] = walker[i]/(g_norm[i-igofr]);
  
  if(print_inst == 1) PrintInstObs();
}
...
void PrintInstObs()
{
  const int wd = 12;
  ofstream of;
  of.open("inst_obs.dat", ios::app);
  
  of << setw(wd) << walker[iv]/(double)npart + vtail << setw(wd) << rho * temp + (walker[iw] + (double)npart * ptail) / vol << endl;
}
...
void Averages(int iblk) //Print results for current block
{
 ...
 //  double r, gdir;
   ...
   const int wd=12;
    ...
    Epot.open("block_epot.dat",ios::app);
    Pres.open("block_pres.dat",ios::app);
    Gofr.open("block_gofr.dat",ios::app);
  ...
  for(int i=igofr; i<n_props; i++){
    stima_g[i-igofr] = blk_av[i]/blk_norm;
    glob_av[i] += stima_g[i-igofr];
    glob_av2[i] += pow(stima_g[i-igofr],2);
    err_g[i-2] = Error(glob_av[i], glob_av2[i], iblk);
  }
